...
function send_email_template2($name, $replaceDict = NULL, $toAddress = NULL, $fromAddress = NULL, $replyAddress = NULL, $bcc = NULL) {
    ...
    function sendEmailTemplate3($name, $replaceDict = NULL, $toAddress = NULL, $fromAddress = NULL, $replyAddress = NULL) {
        $template = runQueryGetFirstRow("SELECT * FROM email_templates WHERE name = " . quoteSQL($name));
        ...
        $message = "<html><body>".nl2br($template['message'])."</body></html>";
        ...
        if (mail3($toAddress, $template['subject'], $body, $headers, "", $name)) {
            return true;
        } else {
            return false;
        }
    ...
    function mail3($toAddress, $subject, $message, $headers, $parameters = "", $template = "") {
        $blocked = false;
        if ($template) {
            $sql = "UPDATE email_templates SET
				count = count + 1
