...
$(function () {
  ...
  $("body").on("click", ".user-listings-link", function (e) {
    e.stopPropagation();
        ...
        if (obj.changed) {
          ...
          $.each(obj.messages, function (message_index, _message) {
            ...
            if (
              ...
              html_final +=
                '<div class="mb-2 text-center text-muted text-sm">- ';
              if (_message.is_lister == "n") {
                if (_message.is_free == "y") {
                  html_final +=
                    'You requested <a title="View listing" href="' +
                    _message.listing_seo +
                    '"><b>' +
                    _message.listing_title +
                    "</b></a> from " +
                    _message.other_firstname;
                } else {
                  html_final +=
                    'You offered <a title="View listing" href="' +
                    _message.listing_seo +
                    '"><b>' +
                    _message.listing_title +
                    "</b></a> to " +
                ...
                if (_message.is_free == "y") {
                  html_final +=
                    _message.other_firstname +
                    ' requested  <a title="View listing" href="' +
                    _message.listing_seo +
                    '"><b>' +
                    _message.listing_title +
                    "</b></a> from you";
                  ...
                  html_final +=
                    _message.other_firstname +
                    ' offered  <a title="View listing" href="' +
                    _message.listing_seo +
                    '"><b>' +
                    _message.listing_title +
                    "</b></a> to you";
            ...
            $(div)
              .append($('<div class="message" />').html(_message.message))
              .append(
                $('<div class="timestamp" />').html(
                  _message.d.capitalizeFirstLetter() + " " + _message.timeAgo,
                ),
              );
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (obj.success) {
        ...
        if (obj.count_unread) {
          $(".html-count_inbox_unread").html(
            " (<strong>" + parseInt(obj.count_unread) + "</strong>)",
          );
...
function limitChars(textid, limit, infodiv) {
  ...
  if (textlength > limit) {
    $("#" + infodiv).html(
      '<span class="errorMsg">You cannot write more than ' +
        limit +
        " characters!</span>",
    );
